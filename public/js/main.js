    ...
    fetch('/api/sorteo-date')
      ...
      .then(data => {
        ...
        if (data.date) {
          ...
          sorteoDateElement.textContent = `Sorteo: ${formatted} (sorteo Astro Luna)`;
          confirmationDateElement.textContent = formatted;
          paymentDateElement.textContent = formatted;
        } else {
          sorteoDateElement.textContent = `Sorteo: ${fallback} (sorteo Astro Luna)`;
      ...
      .catch(err => {
        ...
        sorteoDateElement.textContent = 'Sorteo: Próximamente (sorteo Astro Luna)';
    ...
    fetch('/api/select', {
      ...
      .then(data => {
        if (data.message) {
          ...
          const numerosTexto = selectedNumbers.join(', '); // convierte [12, 34, 56] → "12, 34, 56"
          const mensaje = `Hola, quiero pagar $${total} COP por ${selectedNumbers.length} números (${numerosTexto}). Mis datos son: ${buyerName}, ${buyerPhone}, ${buyerId}. Haré el pago por Nequi y adjuntaré el comprobante.`;
